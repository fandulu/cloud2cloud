...
class CloudDataset(torch.utils.data.Dataset):
    def __init__(self, data, data_met, window=24, horizon=24, dtype=torch.float):
        ...
        self._data_met = data_met['HGTs']
        self.cloud_list = np.stack(list(self._data.keys())).astype(int)
        self.met_list = np.stack(list(self._data_met.keys())).astype(int)
    ...
    def __getitem__(self, index):
        ...
        cloud_met = self._data_met[str(met_ind)]
        ...
        x = np.concatenate([cloud_x,cloud_met],axis=0)
